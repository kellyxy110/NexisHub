...
const countGuideWords = (guide: VisibilityGuide) => [
  guide.lead, ...guide.overview, ...(guide.deepDive ?? []).flatMap((section) => [section.title, ...section.paragraphs]),
  ...guide.principles.flatMap((item) => [item.title, item.detail]), ...guide.workflow.flatMap((item) => [item.title, item.detail]),
  ...guide.pitfalls.flatMap((item) => [item.title, item.detail]), ...guide.measurement, guide.future, ...guide.takeaways,
  ...guide.faq.flatMap((item) => [item.question, item.answer]), ...guide.references.map((item) => item.label),
].join(" ").split(/\s+/).filter(Boolean).length;

const expandShortGuide = (guide: VisibilityGuide): VisibilityGuide => {
  if (countGuideWords(guide) >= 1300) return guide;
  const principleNames = guide.principles.map((item) => item.title.toLowerCase()).join(", ");
  const sections = [
    { title: "Apply the idea to a real page", paragraphs: [`Begin with one page that matters to the organisation and inspect it as a complete information object. Identify its subject, audience, purpose, important claim, supporting evidence, and next action. Then compare those decisions with the page title, main heading, navigation label, summary, links, and structured data. When those layers disagree, repair the underlying meaning before adding more content.`, `For this guide, the first practical pass should examine ${principleNames}. Do not treat the list as a scorecard that produces an authoritative number. Use it to ask which conditions exist, which are uncertain, and which change would make the page more useful to a person as well as a retrieval system.`] },
    { title: "Build an evidence record", paragraphs: [`A useful implementation record names the page or entity, the observation date, the source of the observation, the change made, the expected mechanism, and the limitation that still applies. Technical evidence may include status codes, rendered output, links, metadata, or accessibility results. Editorial evidence may include a source, author, publication date, review decision, or correction record. Keep these classes visible instead of merging them into a single confidence label.`, `The record should also explain what has not been measured. If an article has not been observed in an external answer system, say so. If a recommendation is based on documentation rather than a controlled experiment, say so. Clear limits make a publication more credible because readers can distinguish established practice from a proposal that still needs testing.`] },
    { title: "Diagnose failure before prescribing volume", paragraphs: [`When a page performs poorly in a discovery workflow, classify the failure before recommending more articles. Access problems include blocked routes, unstable responses, rendering gaps, incorrect canonicals, and weak navigation. Interpretation problems include ambiguous names, vague headings, missing definitions, and conflicting descriptions. Evidence problems include unsupported claims, unclear authorship, stale sources, and missing limitations. Each category has a different remedy.`, `A diagnosis should be reproducible by another person. Include the page, question, date, observed result, expected result, and the smallest reasonable next step. This prevents a common editorial failure in which a team publishes volume to compensate for a technical or conceptual problem that the extra pages cannot solve.`] },
    { title: "Make ownership explicit", paragraphs: [`Assign responsibility across the complete lifecycle. Engineering may own rendering, response behaviour, canonical URLs, feeds, and deployment. Content or research may own definitions, sources, examples, and revisions. Product or subject experts may verify capabilities and boundaries. Analytics may preserve samples and distinguish observed outcomes from estimates. A page is more maintainable when these responsibilities are visible.`, `Ownership does not mean every page needs a large process. A small team can use a lightweight review record with an owner, a review date, the evidence checked, and the decision taken. The important point is that no one has to guess who should correct a misleading claim, replace a broken source, or investigate a change in discovery behaviour.`] },
    { title: "Measure useful change", paragraphs: [`Choose a measure that matches the intervention. If the change repairs a canonical, inspect canonical consistency and crawl paths. If it clarifies a definition, review extraction and representation across a fixed question set. If it adds evidence, check whether readers can reach and evaluate the source. If it improves accessibility, test the actual interaction rather than inferring success from the presence of markup.`, `Do not claim a business result from a technical change without a suitable observation window and comparison. Discovery surfaces are variable, and several changes often happen together. Preserve the baseline and describe alternative explanations. A measured improvement can be valuable without being presented as proof that one edit caused every downstream outcome.`] },
    { title: "Maintain the page after publication", paragraphs: [`Publication is the start of a maintenance period, not the end of the work. Review product descriptions when the product changes. Recheck current statistics and specifications on an appropriate interval. Watch for broken links, redirects, withdrawn sources, outdated examples, and new terminology that could confuse the page's identity. Historical sources may remain appropriate; age alone is not a reason to remove them.`, `Keep a version history for material changes. State what changed, why it changed, which sections are affected, and whether the conclusion changed. If a serious error is found, use a correction or retraction process rather than quietly rewriting the old claim. This preserves reader trust and creates a useful record for future research.`] },
    { title: "What would change the conclusion?", paragraphs: [`A strong technical article states the evidence that would support revision. For this subject, that might be a controlled comparison, a larger observation sample, a change in platform documentation, a reproducible failure across several sites, or a source that contradicts the current interpretation. Naming that evidence keeps the article open to improvement rather than turning a practical framework into doctrine.`, `Readers should leave knowing what they can apply now and what still requires validation. The durable recommendation is to improve access, meaning, evidence, and accountability. The uncertain recommendation should remain labelled as uncertain. That distinction is central to responsible content for both humans and machines.`] },
  ] as const;
  return { ...guide, deepDive: [...(guide.deepDive ?? []), ...sections] };
};
